...
// nextBtn.addEventListener('click', () => {
//     socket.emit('nextQuestion', roomInput.value.trim());
//     timerDisplay.innerText = `Time Left: ${timeLeft}s`; // Show the timer when moving to the next question
...
socket.on('nextQuestion', (question) => {
    displayQuestion(question);
    voted = false;
    timerDisplay.style.display = 'block'; // Show the timer for all players
    timerDisplay.innerText = `Time Left: ${timeLeft}s`; // Reset the timer display
});
...
socket.on('updateTimer', (timeLeft) => {
    timerDisplay.innerText = `Time Left: ${timeLeft}s`; // Update the timer display for all clients
    if (timeLeft <= 0) {
        timerDisplay.innerText = ''; // Hide the timer when time runs out
    }
});
